...
\author[Equipo AcademiX]{Equipo AcademiX}
...
\end{columns}
\vspace{0.2cm}
{\footnotesize No competimos por paridad funcional con Canvas o Moodle, sino por integración del flujo crítico.}
\end{frame}
...
\footnotesize Fundaciones \emph{must have}: cursos y secciones, participantes, cuerpo docente visible, evaluaciones y calificaciones. Video, IA, chats, calendario y alertas son \emph{nice to have}: sólo después de validar las fundaciones.
\end{block}
\end{column}
\begin{column}{0.49\textwidth}
\begin{block}{Suposiciones y proceso}
\footnotesize Tenant = universidad; curso = ramo semestral; entregas individuales; escala objetivo de 5 universidades y hasta 30.000 estudiantes cada una. Proceso iterativo semanal con backlog priorizado por riesgo.
\end{block}
\end{column}
\end{columns}
\end{frame}
...
\midrule
Desarrollo & Sobrealcance & No llegar al MVP & Fundaciones primero; extensiones sólo una vez validadas \\
...
\begin{itemize}
  \item \st{Base decidida:} Next.js (cliente web), NestJS (monolito modular), PostgreSQL (notas y estados transaccionales), Docker.
  \item \st{Proveedor decidido:} GCP por su crédito de USD 300 por 3 meses --- Cloud Run, Cloud SQL, Cloud Storage. Componentes portables a otro proveedor.
...
\end{tikzpicture}}\\
\vspace{0.1cm}
{\footnotesize Monolito modular primero. El aislamiento no es una promesa futura: el tenant es el shard desde el primer despliegue.}
\end{frame}
...
{\footnotesize Wireframes navegables en HTML estático: cada vista declara las historias que valida y sirvió para acordar el alcance antes de programar. Notas del estudiante, panel docente y lector de PDF quedan en los anexos.}
\end{frame}
...
8--9 & Libro de notas, promedios, auditoría, dashboard y anuncios (RF14, RF15, RF18, RF19, RF22) & Demo intermedia: docente y estudiante ven la misma verdad \\
...
\begin{itemize}
  ...
  \item Orientan la arquitectura; no exigen demostrar la escala en producción.
\end{itemize}
\end{block}
\end{column}
\begin{column}{0.52\textwidth}
\begin{block}{Escenarios de calidad documentados}
\footnotesize
\begin{itemize}
...
\end{columns}
\vspace{0.3cm}
{\footnotesize Las 37 historias tienen criterios de aceptación y prioridad MoSCoW en \texttt{user-stories.md}. RF1, RF6 y RF19 son habilitadores transversales: se validan a través de las historias de los bloques que los usan.}
\end{frame}
