...
get_current_branch() {
    ...
    if has_git; then
        git -C "$repo_root" symbolic-ref --quiet --short HEAD 2>/dev/null || echo "main"
        return
    fi
...
# Strip a single optional path segment (e.g. gitflow "feat/name" -> "name").
...
check_feature_branch() {
    ...
    local gitflow_type_pattern='^(feat|fix|docs|test|refactor|chore|style|perf|ci|build)/[a-z0-9][a-z0-9-]*$'

    if [[ "$raw" =~ $gitflow_type_pattern ]]; then
        return 0
    fi
    ...
    if [[ "$is_sequential" != "true" ]] && [[ ! "$branch" =~ ^[0-9]{8}-[0-9]{6}- ]]; then
        echo "[specify] Warning: current branch does not follow team GitFlow: $raw" >&2
        echo "[specify] Recommended: feat/short-name, fix/short-name, docs/short-name, etc." >&2
        return 0
    fi
...
find_latest_feature_dir() {
    ...
    local latest=""
    latest=$(find "$specs_dir" -mindepth 1 -maxdepth 1 -type d -printf '%T@ %p\n' 2>/dev/null \
        | sort -nr \
        | head -n 1 \
        | sed 's/^[^ ]* //') || true
...
get_feature_paths() {
    ...
    #   2. .specify/feature.json "feature_directory" key (optional local pointer)
    #   3. Branch-name-based prefix lookup (team convention)
    #   4. Most recently touched specs/* directory (semester-team fallback)
    local feature_dir
    if [[ -n "${SPECIFY_FEATURE_DIRECTORY:-}" ]]; then
        ...
        if [[ -n "$_fd" ]]; then
            ...
            :
        elif feature_dir=$(find_latest_feature_dir "$repo_root"); then
            :
        else
